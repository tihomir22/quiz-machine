...
public class level4 : MonoBehaviour {
	...
	private static float timeforeachquestion =55;
	...
	void Start (){
		...
		PlayerPrefs.SetInt ("RecentLevel", 4);
		PlayerPrefs.Save ();
	...
	void stopgame(){
		...
		PlayerPrefs.SetInt ("level4score", correctanswers);
		PlayerPrefs.Save ();
	...
	void givestars(){
		...
		if (correctanswers >= 12 && correctanswers <= 14) {

			int newstarsearned = 1;
			int previousstars = PlayerPrefs.GetInt("level4stars",0);
		...
		if (correctanswers >= 15 && correctanswers <= 17) {
		...
		if (correctanswers >= 17 && correctanswers <= 20) {
	...
	void reset(){
		...
		timeforeachquestion = 35;
		correctanswers = 0;
		wronganswers = 0;
		unansweredQuestions = null;
		questions = null;

		if(levelloadingcriteria >= 12){
