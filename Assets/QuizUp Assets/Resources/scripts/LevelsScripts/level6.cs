...
public class level6 : MonoBehaviour {
	...
	private static float timeforeachquestion = 30f;
	...
	void Start (){
		...
		PlayerPrefs.SetInt ("RecentLevel", 6);
		PlayerPrefs.Save ();
	...
	void stopgame(){
		...
		PlayerPrefs.SetInt ("level6score", correctanswers);
		PlayerPrefs.Save ();
	...
	void givestars(){
		...
		if (correctanswers >= 14 && correctanswers <= 16) {

			int newstarsearned = 1;
			int previousstars = PlayerPrefs.GetInt("level6stars",0);
		...
		if (correctanswers >= 17 && correctanswers <= 18) {
